...
All external API calls live here. The pipeline notebook imports from
this module so that data-fetching logic is never mixed with modelling
or plotting code.
...
def fetch_weather_from_mesonet(station: str, start: str, end: str,
                                ...
                                planting_date: str = None) -> tuple[pd.DataFrame, list[dict]]:
    """
    ...
    variables = ",".join([
        "TEMP2MMIN", "TEMP2MMAX", "TEMP2MAVG", "PRECIP", "SR",
        "WSPD2MAVG", "VPDEFAVG",
    ])
    url = (
    ...
    try:
        df = pd.read_csv(url, na_values=["M"], parse_dates=["TIMESTAMP"])
    except Exception as exc:
        raise RuntimeError(f"Mesonet fetch failed for station '{station}': {exc}") from exc

    # ── rename & drop unneeded columns ──────────────────────────────────────
    df = df.drop(columns=["STATION", "VPDEFAVG", "WSPD2MAVG"], errors="ignore")
    ...
    weather_data = df.to_dict("records")   # fast list-of-dicts for daily step()

    print(
        f"Weather loaded: {len(weather_data)} days  "
